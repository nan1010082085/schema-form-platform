...
describe('FgBarChart', () => {
  ...
  function mountChart(overrides: Record<string, unknown> = {}) {
    ...
    return mount(FgBarChart, {
      global: {
        plugins: [ElementPlus],
        provide: {
          [widgetDataKey as symbol]: computed(() => store.findWidget('test_bar_chart')!),
          [widgetStyleKey as symbol]: computed(() => store.findWidget('test_bar_chart')!.style ?? {}),
        },
      },
    })
  }
  ...
  describe('属性', () => {
    ...
    it('props 可覆盖', () => {
      const widget = createWidget('bar-chart', 'test_bar_chart')!
      store.addWidget(widget)
      store.updateWidget('test_bar_chart', { props: { xField: 'name', yField: 'count', title: 'Test Title' } })
  ...
  describe('事件', () => {
    ...
    it('支持 set-data 事件配置', () => {
      ...
      store.addWidget(widget)
      expect(store.findWidget('test_bar_chart')!.events).toHaveLength(1)
      expect(store.findWidget('test_bar_chart')!.events![0].trigger).toBe('set-data')
    })
  })
  ...
  describe('数据源', () => {
    it('支持 api 配置', () => {
      ...
      store.addWidget(widget)
      expect(store.findWidget('test_bar_chart')!.api!.url).toBe('/api/bar-data')
    })
  })
