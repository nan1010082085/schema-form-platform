...
<template>
  <div v-if="entries.length > 0" :class="styles.root">
    <el-table :data="entries" stripe :class="styles.table" size="small">
      ...
      <el-table-column label="最新值" min-width="200">
        <template #default="{ row }">
          <el-tooltip
      ...
      <el-table-column label="历史" width="100" v-if="entries.some(e => isExpandable(e.value))">
        <template #default="{ row }">
          <span
            v-if="isExpandable(row.value)"
