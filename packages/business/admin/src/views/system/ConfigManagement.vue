...
import { loadConfigs, createConfig, updateConfig, deleteConfig } from '@/api/adminApi'
...
async function load() {
  loading.value = true
  try { const d = await loadConfigs(); configs.value = d.items } catch { } finally { loading.value = false }
}
...
async function handleSave() {
  ...
  try {
    if (editingConfig.value) await updateConfig(editingConfig.value.id, form.value)
...
async function handleDelete(config: any) {
  try {
    ...
    await deleteConfig(config.id); ElMessage.success('删除成功'); await load()
