...
import { startInstanceSchema } from '../schemas/instanceSchemas.js';
...
router.get('/', requireAuth, async (ctx) => {
    ...
    const filter = {};
    if (definitionId && uuidValidate(definitionId))
        filter.definitionId = definitionId;
    if (status &&
        ['running', 'completed', 'terminated', 'suspended', 'failed'].includes(status)) {
...
router.post('/', requireAuth, validate(startInstanceSchema), async (ctx) => {
    const { definitionId, variables } = ctx.request.body;
